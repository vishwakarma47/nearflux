...
function generateRoomCode(): string { return `FLUX-${Math.random().toString(36).slice(2, 6).toUpperCase()}`; }
...
class BridgeSession {
  ...
  constructor(
    ...
    this.socket = io(socketUrl, { transports: ['websocket'], reconnection: true, autoConnect: false, auth: bridgeSecret ? { bridgeSecret } : undefined });
...
export class TelegramBridgeManager {
  ...
  constructor(private readonly config: { token: string; socketUrl: string; webAppUrl: string; webhookSecret?: string; bridgeSecret?: string; publicUrl?: string }) {
  ...
  private async configureWebhook(): Promise<void> {
    try { await this.api.setMyCommands(); const publicUrl = this.config.publicUrl || process.env.BRIDGE_PUBLIC_URL || process.env.TELEGRAM_WEBHOOK_URL || process.env.RENDER_EXTERNAL_URL || process.env.CLIENT_ORIGIN; if (publicUrl) await this.api.setWebhook(`${publicUrl.replace(/\/$/, '')}/telegram/webhook`, this.config.webhookSecret); console.log('[telegram] bot configured'); } catch (error) { console.warn(`[telegram] configuration failed: ${error instanceof Error ? error.message : String(error)}`); }
  }

  private session(chatId: number): BridgeSession {
    let session = this.sessions.get(chatId);
    if (!session) {
      session = new BridgeSession(chatId, this.api, this.config.socketUrl, this.config.bridgeSecret, async (text) => { await this.api.sendMessage(chatId, text); }, async (bytes, name, type) => {
...
export function createTelegramBridge(app: Express, port: number): TelegramBridgeManager | null {
  ...
  const manager = new TelegramBridgeManager({
    token,
    webAppUrl: webApp,
    socketUrl: process.env.BRIDGE_SOCKET_URL || process.env.NEARFLUX_SIGNALING_URL || process.env.NEARFLUX_INTERNAL_URL || `http://127.0.0.1:${port}`,
    webhookSecret: process.env.TELEGRAM_WEBHOOK_SECRET,
    bridgeSecret: process.env.BRIDGE_SHARED_SECRET,
    publicUrl: process.env.BRIDGE_PUBLIC_URL || process.env.TELEGRAM_WEBHOOK_URL,
  });
