  ...
  return (
    <>
      <header className="app-header">
        <div className="logo-container"><div className="logo-icon-bg" aria-hidden="true"><Wifi size={21} strokeWidth={2.2} /></div><div className="logo-text"><h1>NearFlux</h1><span className="logo-tagline">Direct P2P file transfer</span></div></div>
        <div className="header-actions">
          ...
          <span className="guest-mode-pill" title="Transfer history is saved in this browser"><UserRound size={14} aria-hidden="true" /><span>Guest</span></span>
          {roomCode && <button className="device-badge room-pill" type="button" onClick={() => setIsRoomModalOpen(true)} title="Open room sharing" aria-label={`Open sharing for room ${roomCode}`}><LockKeyhole size={14} aria-hidden="true" /><span>Room <RoomCode code={roomCode} /></span><QrCode size={13} aria-hidden="true" /></button>}
          ...
          <button className="help-btn" type="button" onClick={() => setIsHelpOpen((open) => !open)} title="How to use" aria-label="How to use NearFlux" aria-expanded={isHelpOpen}><HelpCircle size={17} aria-hidden="true" /></button>
          <ThemeToggle />
        </div>
